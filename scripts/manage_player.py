"""
manage_player.py
Manually create or update a player profile via terminal prompts.

Mirrors the LOCAL_CSV_INGEST logic — safe to re-run on existing players.

Usage:
    python3 manage_player.py
"""
...
def ask_rank_full(label: str, required: bool = False) -> str | None:
    """
    Like ask_rank() but also accepts LP inline.
    e.g. 'E4 98 LP' -> 'Emerald 4 98 LP'  |  'G2' -> 'Gold 2'  |  blank -> None
    """
...
def add_account_automated(profile, registry: PlayerRegistry) -> None:
    riot_id, region = ask_riot_id()
    archived_raw = input("  Archived (banned/inactive)? [y/N]: ").strip().lower()
    archived = archived_raw == "y"
...
def update_existing_accounts_automated(profile, registry: PlayerRegistry) -> None:
...
def add_tournament_season(profile, registry: PlayerRegistry) -> None:
...
def _resolve_season_entry(profile, action_label: str):
...
def manage_adjustments(profile, registry: PlayerRegistry) -> None:
    season_entry = _resolve_season_entry(profile, "adjustments")
...
def enter_inhouse_data(profile, registry: PlayerRegistry) -> None:
    season_entry = _resolve_season_entry(profile, "in-house data")
    if season_entry is None:
        return
    season = season_entry.season
        ...
        try:
            ...
            if profile:
                ...
                if action == "Update existing accounts (OP.GG scraper)":
                    update_existing_accounts_automated(profile, registry)
                    ...
                    if sub == "Manual entry":
                        ...
                        add_account_automated(profile, registry)

                elif action == "Add new tournament season":
                    add_tournament_season(profile, registry)

                elif action == "Manage adjustments":
                    manage_adjustments(profile, registry)

                elif action == "Enter in-house data":
                    enter_inhouse_data(profile, registry)
